...
generate_temp_yaml = lambda path: shutil.copy2(
    path, os.path.join(tempfile.gettempdir(), "osrepo.tmp.yaml"))
request_download = lambda filename, path: open(filename, 'wb').write(
    requests.get(path, allow_redirects=True).content)
...
def initialize():
    ...
    request_download(
        "VARIABLES",
        "https://raw.githubusercontent.com/FreeTurk/OSRepo/stable/VARIABLES")
    # parse VARIABLES
...
def get_command(user):
    if user.startswith("search "):
        ...
        i = 0
        for some_os in all_oses_list:
            i += 1
            print("({}){}".format(i, some_os))
    elif user == "download":
        clear()
        walk(repo)
    elif user == "help":
